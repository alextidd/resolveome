          ...
          size_ticks = 0.5) 
{
  theme(strip.background.x = element_blank(), strip.background.y = element_blank(), 
        ...
        panel.background = element_rect(fill = "white"), panel.grid.major.x = element_line(color = "grey89", 
                                                                                           size = 0.25), panel.grid.major.y = element_line(color = "grey89", 
                                                                                                                                           size = 0.25), panel.grid.minor.y = element_blank(), 
        panel.grid.minor.x = element_blank(), panel.border = element_rect(fill = NA, 
                                                                          color = "black", size = size_panel_border), axis.line = element_blank(), 
        axis.ticks = element_line(colour = "black", size = size_ticks), 
        axis.text.x = element_text(family = font_family, face = font_face, 
                                   size = size_axis_text.x, colour = "black", hjust = x_hjust, 
                                   angle = angle_text.x), axis.text.y = element_text(family = font_family, 
                                                                                     face = font_face, size = size_axis_text.y, colour = "black", 
                                                                                     vjust = y_vjust, angle = angle_text.y), axis.title.x = element_text(family = font_family, 
                                                                                                                                                         face = font_face, size = size_axis_title.x, colour = "black"), 
        axis.title.y = element_text(family = font_family, face = font_face, 
                                    size = size_axis_title.y, colour = "black"), legend.background = element_blank(), 
        legend.key.size = unit(legend_proportion_size, "line"), 
        legend.title = element_text(size = size_title_text, family = font_family, 
                                    face = font_face, colour = "black"), legend.key = element_blank(), 
        legend.text = element_text(size = size_legend_text, family = font_family, 
                                   face = font_face, colour = "black"), legend.position = "right", 
...
df_freq <- df_placed$Branch %>% table %>%
...
df_vaf <- merge(df_nv,df_nr, by = c("Var1","Var2")) %>% dplyr::mutate(.data =.,VAF = value.x/value.y) %>%
...
mnums <- df_placed[,c("VariantId","Branch")] %>% unique %$% Branch %>% table
...
df_vaf_int <- df_vaf %>% dplyr::filter(.data =.,PlacedNodeId %in% order_nodes) %>% droplevels
...
order_nl <- with(df_vaf_int,order(PlacedNodeId)) %>% df_vaf_int$LabelFacet[.] %>% unique
...
composition <- egg::ggarrange(p_tree,p_vaf_branches,nrow = 1,widths  = c(0.4,1),draw = TRUE)
dev.off()
...
oh.save.pdf(p = composition,outname = "res_composition.pdf",outdir = "./",width = tot_width,height = tot_height)


#Save vaf table
write.table(x = df_vaf,file = "res_vaf.tsv",append = FALSE,quote = FALSE,sep = "\t",row.names = FALSE,col.names = TRUE)
...
saveRDS(object = Dat,file = "res_vaf_ptree.RDS")
