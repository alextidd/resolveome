...
man_insp <-
  ...
  select(cell_id, starts_with("celltype_"), TNFRSF14_mut, loh_1p) %>%
  distinct() 
```
...
# dirs
bje_dir <- "out/BaseJumper/bj-expression/filter_cells/PD63118_250329_235910/"
...
```{r}
counts_file <- "out/BaseJumper/bj-expression/filter_cells/PD63118_250329_235910/secondary_analyses/quantification_htseq/matrix_gene_counts_starhtseq.txt"
...
cell_stats <-
  ...
  dplyr::mutate(cell_id = rep(colnames(expr)[-c(1, 2)], 2)) # Add cell labels

# Plot
cell_stats %>%
  dplyr::inner_join(man_insp) %>%
  dplyr::group_by(cell_id) %>%
  dplyr::mutate(arr = max(value)) %>%
  dplyr::ungroup() %>%
...
cells <-
  man_insp %>%
  dplyr::filter(celltype_VDJ_recomb == "B cell") %>%
  dplyr::rowwise() %>%
