...
# get latest bj dir
bj_dir <-
  list.files("out/BaseJumper/bj-expression/", pattern = "PD63118_",
...
cts$bj <-
  ...
  dplyr::select(cell_id, celltype_bj = Progenitor, tcga_tissue = TGCA_Tissue)
```
...
cts %>%
  ...
  dplyr::count(celltype_bj, group) %>%
  ggplot(aes(x = reorder(celltype_bj, -n), y = n, fill = n)) +
  geom_col() +
