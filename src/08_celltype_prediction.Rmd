...
```{r cts_bj}
cts$bj <-
  readr::read_tsv("out/BaseJumper/bj-expression/PD63118_250226_100238/tertiary_analyses/classification_cell_typing/df_cell_typing_summary_singler_hpca_gtex_tcga.tsv") %>%
  dplyr::left_join(ss, by = c("SampleId" = "id")) %>%
  dplyr::select(cell_id, celltype_bj = Progenitor)

cts$bj %>%
  dplyr::inner_join(ss_clean) %>%
  dplyr::count(celltype_bj) %>%
  ggplot(aes(x = reorder(celltype_bj, -n), y = n)) +
  geom_col() +
